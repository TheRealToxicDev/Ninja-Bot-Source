...
exports.run = (client, message, args) => {
    const user = message.mentions.users.first();
     if (message.mentions.users.size < 1) return message.channel.send("Need to tag a user to take money from them.");
    ...
    sql.get(`SELECT * FROM profiles WHERE guildId ="${message.guild.id}" AND userId ="${user.id}"`).then(row => {
     if (!row) message.channel.send("User needs to start talking first.")
		...
		message.channel.send("I have taken money from: " + user.username + " $" + amount);
    })
}
